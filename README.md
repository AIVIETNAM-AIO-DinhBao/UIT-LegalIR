...
The offline notebook loads the model snapshots by local path and sets `HF_HUB_OFFLINE=1` and `TRANSFORMERS_OFFLINE=1`. It creates a fresh virtual environment under `/kaggle/working` with `--system-site-packages`, installs only from the attached wheelhouse with `--no-index`, and executes both model preflight and every LegalIR pipeline stage through that environment's Python. The long-lived notebook kernel never imports the model stack, so packages left in Kaggle's kernel from an earlier cell cannot contaminate the run. The environment still inherits Kaggle's CUDA-compatible PyTorch, NumPy, SciPy, and scikit-learn instead of replacing those large compiled packages.
